...
$svenConversationData = file_get_contents('JSONdata/sven.json', 'r');
$svenConversationDataArray = json_decode($svenConversationData, true);
...
for($i = count($svenConversationDataArray)-1; $i >= 0 ; $i--) {
	foreach($svenConversationDataArray[$i] as $key => $value) {
