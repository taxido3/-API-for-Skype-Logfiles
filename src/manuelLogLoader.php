...
$manuelConversationData = file_get_contents('JSONdata/manuel.json', 'r');
$manuelConversationDataArray = json_decode($manuelConversationData, true);
...
for($i = count($manuelConversationDataArray)-1; $i >= 0 ; $i--) {
	foreach($manuelConversationDataArray[$i] as $key => $value) {
