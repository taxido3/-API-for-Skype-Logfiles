...
$paulConversationData = file_get_contents('JSONdata/paul.json', 'r');
$paulConversationDataArray = json_decode($paulConversationData, true);
...
for($i = count($paulConversationDataArray)-1; $i >= 0 ; $i--) {
	foreach($paulConversationDataArray[$i] as $key => $value) {
